...
# [COMPONENT]
# [CONFIG|POST_INSTALL|POST_UNINSTALL]
...
MACRO(MYSQL_ADD_PLUGIN)
  CMAKE_PARSE_ARGUMENTS(ARG
    ...
    "MODULE_OUTPUT_NAME;COMPONENT;CONFIG;POST_INSTALL;POST_UNINSTALL"
  ...
  INCLUDE_DIRECTORIES(${CMAKE_SOURCE_DIR}/include ${CMAKE_SOURCE_DIR}/sql
                      ${PCRE_INCLUDES} ${SSL_INCLUDE_DIRS} ${ZLIB_INCLUDE_DIR})
  ...
  IF (ARG_STORAGE_ENGINE)
    MESSAGE(WARNING "MYSQL_ADD_PLUGIN(${plugin}): STORAGE_ENGINE clause is obsolete")
  ENDIF()
  IF (CMAKE_CURRENT_SOURCE_DIR MATCHES "^${CMAKE_SOURCE_DIR}/plugin/")
    SET(ARG_STORAGE_ENGINE FALSE)
  ELSE()
    SET(ARG_STORAGE_ENGINE TRUE)
  ENDIF()
    ...
    IF(ARG_COMPONENT AND CPACK_COMPONENTS_ALL)
      IF(NOT CPACK_COMPONENTS_ALL MATCHES ${ARG_COMPONENT}
        ...
        IF(UNIX)
          ...
          ENDIF()
          INSTALL(FILES ${ARG_CONFIG} COMPONENT ${ARG_COMPONENT} DESTINATION ${INSTALL_SYSCONF2DIR} RENAME "${target}.cnf")
          ...
          SET(postin "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/plugin-postin.sh")
          CONFIGURE_FILE(${CMAKE_SOURCE_DIR}/support-files/rpm/plugin-postin.sh.in ${postin} @ONLY)
          SET(CPACK_RPM_${ARG_COMPONENT}_POST_INSTALL_SCRIPT_FILE ${postin} PARENT_SCOPE)
        ENDIF()
      ENDIF()
    ELSE()
      SET(ARG_COMPONENT Server)
  ...
  IF (TARGET ${target})
    IF (ARG_POST_INSTALL MATCHES "\\.sql$")
      INSTALL(FILES ${ARG_POST_INSTALL} COMPONENT ${ARG_COMPONENT}
        DESTINATION ${INSTALL_MYSQLSHAREDIR} RENAME "${target}-postin.sql")
    ELSEIF(ARG_POST_INSTALL)
      MESSAGE(FATAL_ERROR "Only *.sql postinstall scriptlets are supported")
    ENDIF()
