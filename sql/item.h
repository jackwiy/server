...
public:
  ...
  */
  ...
  /*
    TRUE if the expression is an item used in the GROUP BY of sl, constant
    or item that is functionally dependent on field(s) that are allowed to be
    used in the SELECT sl.
  ...
  */
  ...
  /*
    ...
    fields that are functionally dependent on fields that are allowed to be
    used in SELECT sl (1). Also another condition should be met:
    expression and its parts can be used to pull out a new functionally
    dependent field. This field is extracted from the WHERE clause equality.
    Collect in 'fields' argument fields used in this expression.
    Store in 'item' argument item that doesn't meet (1). This special case can
    be met when sl is a subquery and forbidden outer reference is used
    in the WHERE clause of sl.
    It is forbidden to use the same functions that are forbidden
    to be used in propagation of equal fields.
  */
  virtual bool excl_func_dep_from_equalities(st_select_lex *sl,
                                             Item **item,
                                             List<Field> *fields)
  {
    if (const_item())
  ...
  virtual bool set_subquery_ctx(void *arg)
  { return false; }
  ...
  */
  ...
  bool is_group_by_item(List<Item> *det_items)
  {
    if (det_items->is_empty())
      return false;
    List_iterator<Item> it(*det_items);
    Item *item_arg;
    while ((item_arg= it++))
...
protected:
                                        ...
                                        Item **item)
  {
    for (uint i= 0; i < arg_count; i++)
    {
      if (!args[i]->excl_func_dep_on_grouping_fields(sl, det_items, item))
        return false;
    }
    return true;
  }
                                     ...
                                     List<Field> *fields)
  {
    ...
    {
      bool dep_arg=
        args[i]->excl_func_dep_from_equalities(sl, item, fields);
...
public:
  ...
  */
  ...
  bool excl_func_dep_on_grouping_fields(st_select_lex *sl,
                                        ...
                                        Item **item)
  {
    if (field->excl_func_dep_on_grouping_fields(sl, det_items, item))
      return true;
    *item= this;
    return false;
  }
...
public:
  ...
  */
  ...
  bool excl_func_dep_on_grouping_fields(st_select_lex *sl,
                                        ...
                                        Item **item)
  {
    if (!(*ref)->excl_func_dep_on_grouping_fields(sl, det_items, item))
    {
  ...
  bool excl_func_dep_from_equalities(st_select_lex *sl,
                                     Item **item,
                                     List<Field> *fields)
  { return (*ref)->excl_func_dep_from_equalities(sl, item, fields); }
};
